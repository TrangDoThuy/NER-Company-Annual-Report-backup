...
#regex = re.compile(r'(Business(.)*</)|(Risk Factors(\s)*</)|(Risk Factors[^</]*</)')
...
if(len(list(matches))>0):
    matches = regex.finditer(item_1_raw)
    test_df = pd.DataFrame([( x.start(), x.end(),item_1_raw[x.start():x.end()]) for x in matches])
    ...
    for match in matches:
        ...
        new_row = {'start':start_index,'end':end_index,'header':item_1_raw[start_index:end_index]}
        data.append(new_row)
    test_df = pd.DataFrame(data)
print("*"*10 )
print(test_df)
    ...
    for title in common_title:
        if title in object_list[i]["header"].lower():
            ...
            if(len(object_list[i]["content"])>0):
                #print(object_list[i])
                if "\n" in object_list[i]["content"]:
                    index = object_list[i]["content"].find("\n")
                    overview = object_list[i]["content"][:index]
                    business_review = object_list[i]["content"][index:]
                    has_business_title = True
                else:
...
if(not has_overview_title):
    ...
    if "\n" in object_list[first_part-1]["content"]:
        index = object_list[first_part-1]["content"].find("\n")
        overview = object_list[first_part-1]["content"][:index]
        business_review = object_list[first_part-1]["content"][index:]
        has_business_title = True
    else:
        overview = object_list[first_part-1]["content"]

#%%
if(not has_business_title):
    for i in range(1,len(object_list)):
        
        if "business" in object_list[i]["header"].lower():
            #print()
            has_overview_title = True
            if(len(object_list[i]["content"])>0):
                #print(object_list[i])
...
#%%
with open('header_content_American_Airlines_Group_Inc.json', 'w', encoding='utf-8') as f:
