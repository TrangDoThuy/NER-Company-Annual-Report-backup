...
def model_info_extraction(file_path):
    ...
    def remove_unnecessary_letter(input_string):
        ...
        # if "testing; vacuum" in input_string:
        #     print("////////////////////////////////")
        #     print(input_string)
    ...
    def extract_item_1_business(file_path):
        ...
        regex = re.compile(r'(>Item(\s|&#160;|&nbsp;)(1A|2|1.)\.{0,1})|(>Item(1A|2|1.)\.{0,1})|(>(\s)+Item(\s|&#160;|&nbsp;)(1A|2|1.)\.{0,1})|(>(\s)+Item(1A|2|1.)\.{0,1})|(>ITEM(\s|&#160;|&nbsp;)(1A|2|1.)\.{0,1})|(>ITEM(1A|2|1.)\.{0,1})|(>(\s)+ITEM(\s|&#160;|&nbsp;)(1A|2|1.)\.{0,1})|(>(\s)+ITEM(1A|2|1.)\.{0,1})|(>ITEM[^<]+<)|(>Item[^<]+<)|(>ITEM[^2]+2.)|(>Item[^2]+2.)|(>ITEM[^1]+1.)|(>Item[^1]+1.)|(>ITEM[^1]+1A.)|(>Item[^1]+1A.)')
    ...
    def extract_item_7_financial_analyse(file_path):
        f=codecs.open(file_path, 'r')
        file_content = f.read()
        regex = re.compile(r'(>Item(\s|&#160;|&nbsp;)(7A|7|7.)\.{0,1})|(>Item(7A|8|7.)\.{0,1})|(>(\s)+Item(\s|&#160;|&nbsp;)(7A|8|7.)\.{0,1})|(>(\s)+Item(7A|8|7.)\.{0,1})|(>ITEM(\s|&#160;|&nbsp;)(7A|8|7.)\.{0,1})|(>ITEM(7A|8|7.)\.{0,1})|(>(\s)+ITEM(\s|&#160;|&nbsp;)(7A|8|7.)\.{0,1})|(>(\s)+ITEM(7A|8|7.)\.{0,1})|(>ITEM[^<]+<)|(>Item[^<]+<)')
        matches = regex.finditer(file_content)
        item_7_raw = ""
        # Create the dataframe
        if(len(list(matches))>0):
    ...
    def generate_header_content(raw_text):
        ...
        # Write the regex
        regex = re.compile(r'(<B><U>[^<]+</U></B>)|(<B><I>[^<]+</I></B>)|(<B>[^<]+</B>)|(<b>(.*)</b>)|(<b><u>(.*)</u></b>)|(<b><i>(.*)</i></b>)|(bold[^<]+</)|(font-weight:700[^<]+</)|(font-weight: 700[^<]+</)')
        #regex = re.compile(r'(<B><I>[^<]+</I></B>)|(<B>[^<]+</B>)')
   ...
   # print("Overview")
    ...
    return json_object  
#%%
# file_path = 'flask_NER/static/Data/NaN/Asset Acceptance Capital Corp/annual_report_2013-04-26_d527278d10ka.htm' 
